...
namespace automatic_start_pointer
{

/* class AutomaticStartPointer //{ */
...
class AutomaticStartPointer : public nodelet::Nodelet {
...
void AutomaticStartPointer::onInit() {
  ...
  mrs_lib::ParamLoader param_loader(nh_, "AutomaticStartPointer");
  ...
  subscriber_mavros_state    = nh_.subscribe("mavros_state_in", 1, &AutomaticStartPointer::callbackMavrosState, this, ros::TransportHints().tcpNoDelay());
  subscriber_tracker_status  = nh_.subscribe("tracker_status_in", 1, &AutomaticStartPointer::callbackTrackerStatus, this, ros::TransportHints().tcpNoDelay());
  subscriber_mpc_diagnostics = nh_.subscribe("mpc_diagnostics_in", 1, &AutomaticStartPointer::callbackMpcDiagnostics, this, ros::TransportHints().tcpNoDelay());
  ...
  main_timer = nh_.createTimer(ros::Rate(main_timer_rate_), &AutomaticStartPointer::mainTimer, this);
  ...
  ROS_INFO("[AutomaticStartPointer]: initialized");
}
...
void AutomaticStartPointer::callbackMavrosState(const mavros_msgs::StateConstPtr& msg) {
  ...
  ROS_INFO_ONCE("[AutomaticStartPointer]: getting mavros state");
...
void AutomaticStartPointer::callbackTrackerStatus(const mrs_msgs::TrackerStatusConstPtr& msg) {
  ...
  ROS_INFO_ONCE("[AutomaticStartPointer]: getting tracker status");
...
void AutomaticStartPointer::callbackMpcDiagnostics(const mrs_msgs::TrackerDiagnosticsPtr& msg) {
  ...
  ROS_INFO_ONCE("[AutomaticStartPointer]: getting mpc diagnostics");
...
void AutomaticStartPointer::mainTimer([[maybe_unused]] const ros::TimerEvent& event) {
  ...
  switch (current_state) {
    ...
    case TAKEOFF_STATE: {
      ...
      if (tracker_status.tracker.compare(std::string("MpcTracker")) == STRING_EQUAL) {
        ...
        ROS_INFO("[AutomaticStartPointer]: takeoff finished");
        ...
        ROS_INFO("[AutomaticStartPointer]: calling goto");
    ...
    case GOTO_STATE: {
      ...
      if (!mpc_diagnostics.tracking_trajectory) {
        ...
        ROS_INFO("[AutomaticStartPointer]: reached goal, landing");

        std_srvs::Trigger trigger_out;
        service_client_land.call(trigger_out);
...
PLUGINLIB_EXPORT_CLASS(mrs_general::automatic_start_pointer::AutomaticStartPointer, nodelet::Nodelet)
